...
struct DEV_LED : Service::LightBulb
{
	...
	DEV_LED(int ledPin, TFT_eSPI* ptft) : Service::LightBulb()
	{
		this->ptft = ptft;
...
void Display(char* pText)
{
	...
	//////////////////////////////////////////////////////////////////////////////
	// Finally, we over-ride the default update() method with instructions that 
	// actually turn on/off the LED.  Note update() returns type boolean
	boolean update()
	{
		pinState = power->getNewVal();
		digitalWrite(ledPin,pinState);        // use a standard Arduino function to turn on/off ledPin based on the return of a call to power->getNewVal() (see below for more info)
